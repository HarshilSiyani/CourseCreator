class Chat::MessagesController < ApplicationController
  def create
    @course = Course.includes(:students).find(params[:course_id])
    ...
    if @message.save
      ...
      NotificationChannel.broadcast_to(@course.teacher,
        # messageHtml: render_to_string(partial: "chat/messages/message", locals: { message: @message }),
        notificationHtml: render_to_string(partial: "chat/messages/notification", locals: { message: @message, course: @course })
      )
      @course.students.each do |student|
        NotificationChannel.broadcast_to(student,
          # messageHtml: render_to_string(partial: "chat/messages/message", locals: { message: @message }),
          notificationHtml: render_to_string(partial: "chat/messages/notification", locals: { message: @message, course: @course })
        )
      end
